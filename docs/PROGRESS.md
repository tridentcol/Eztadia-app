...
>
> Última actualización: 2026-05-27 — **Step 15** ✅ Brand Horizonte + sección Deudas + dashboard breakdown. Tres commits: `aab7271` (integración marca: símbolo SVG + wordmark Sora 500, tokens `--brand-purple-*`, accent-ai lavanda intacto solo para IA, hover/active sidebar con tinte morado sutil, favicon SVG, landing rediseñada con watermark + pilares + banda tintada). `ac49982` (tabla `debts` con RLS aplicada vía Supabase MCP, página `/deudas` con tarjetas + préstamos + próximo pago, `DebtsSummaryCard` y `CategoryBreakdown` con barra apilada en familia morada en dashboard, removida flag `experimental.viewTransition` residual, BrandMark centrado al colapsar sidebar). **Pendiente**: pegar `OPENAI_API_KEY` + `ANTHROPIC_API_KEY` (opcional) en Vercel; switch Clerk prod cuando se quiera; QA pass.
