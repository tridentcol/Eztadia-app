...
>
> Última actualización: 2026-05-27 — **Step 16** ✅ Pasada de performance triple. Tres commits: `7bf55b3` (batching de tasas + lazy dialogs vía Suspense + 13 loading.tsx por ruta), `7f6c705` (pin Vercel a `pdx1` para co-locarse con Supabase us-west-2 — ~240ms menos por navegación) y `4760b34` (prefetch agresivo en mobile bottom-nav y sheet "Más" para acercar el feel desktop). Usuario confirma: desktop "mucho mejor" tras 7bf55b3, mobile "más fluido" tras 7f6c705, y al cerrar 4760b34 marca "Excelente". **Pendiente**: pegar `OPENAI_API_KEY` + `ANTHROPIC_API_KEY` (opcional) en Vercel; switch Clerk prod cuando se quiera; QA pass.
