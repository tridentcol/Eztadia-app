...
export async function createTransaction(
  ...
  // Verificar cuenta destino si transfer
    ...
    if (targetAccount.currency !== sourceAccount.currency) {
      return {
        ...
        error: {
          ...
          message:
            'Las transferencias entre monedas distintas no están soportadas todavía.',
        },
      }
    }
  }

  // Verificar categoría (sistema o del usuario)
  if (data.categoryId) {
  ...
  // Compute amount_base
  // TODO Step 8: usar tasa real de exchange_rates si currency != baseCurrency.
  // Hasta entonces: si coincide con base, copia; si no, mock 1:1.
  const profile = await db.query.profiles.findFirst({
    where: eq(profiles.userId, user.id),
  })
  const baseCurrency = profile?.baseCurrency ?? 'COP'
  const amountBase =
    data.currency === baseCurrency ? data.amountOriginal : data.amountOriginal
  const exchangeRate = data.currency === baseCurrency ? '1.000000' : '1.000000'
